...
export async function generateMetadata({params}:{params:Promise<{slug:string}>}){
  ...
  return p?{title:`${p.title} — Naeem Al-Awartany`,description:p.statement,openGraph:{title:p.title,description:p.statement,images:[sitePath(p.hero)]},twitter:{card:'summary_large_image',title:p.title,description:p.statement,images:[sitePath(p.hero)]}}:{};
}

const CaseHeader=({p}:{p:Project})=><>
  <header className="case-nav"><a href={sitePath("/")} className="mark"><span>NA</span><small>NAEEM AL-AWARTANY<br/>ARCHITECTURE + MATTER</small></a><a href={sitePath("/#work")}>Close project ×</a></header>
  <section className="case-hero"><img src={sitePath(p.hero)} alt={`${p.title} architectural overview`}/><div className="case-overlay"><p>{p.type.toUpperCase()} · {p.year}</p><h1>{p.title}</h1><span>{p.category}{p.location&&` · ${p.location}`}</span></div></section>
</>;
...
function FireStation({p}:{p:Project}){return <main className="case thesis-case">
  ...
  <section className="renders"><header className="gallery-head"><div className="section-label"><b>06</b><span>Selected Images</span></div><p>Overview / elevation / materiality</p></header><div className="render-gallery">{p.gallery.map((image,i)=><figure key={image} className={i===0?'wide':''}><img src={sitePath(image)} alt={`Fire Station ${['project overview','main architectural elevation and full-building view','façade detail and materiality'][i]}`}/><figcaption>{['PROJECT OVERVIEW','MAIN ELEVATION + FULL-BUILDING VIEW','FAÇADE DETAIL + MATERIALITY'][i]}</figcaption></figure>)}</div></section>
  ...
  <a className="next-project" href={sitePath("/projects/terraced-community-park")}><span>Next project</span><h2>Terraced Community Park ↗</h2></a>
</main>}
...
function TerracedCommunityPark({p}:{p:Project}){return <main className="case thesis-case">
  ...
  <section className="renders"><header className="gallery-head"><div className="section-label"><b>03</b><span>Selected Landscape Views</span></div><p>Terraces / paths / gathering / play / sport</p></header><div className="render-gallery">{p.gallery.map((image,i)=><figure key={image} className={(i===0||i===3)?'wide':''}><img src={sitePath(image)} alt={`Terraced Community Park ${['seating and pedestrian paths','shaded social gathering space','children’s play area','sports and basketball zone'][i]}`}/><figcaption>{['SEATING + PEDESTRIAN PATHS','SHADED SOCIAL + PERGOLA SPACE','CHILDREN’S PLAY AREA','SPORTS + BASKETBALL ZONE'][i]}</figcaption></figure>)}</div></section>
  ...
  <a className="next-project" href={sitePath("/projects/integrated-urban-district")}><span>Next project</span><h2>Integrated Urban District ↗</h2></a>
</main>}
...
function IntegratedUrbanDistrict({p}:{p:Project}){return <main className="case thesis-case">
  ...
  <section className="renders"><header className="gallery-head"><div className="section-label"><b>03</b><span>Selected Urban Views</span></div><p>Circulation / mixed use / housing / landscape</p></header><div className="render-gallery">{p.gallery.map((image,i)=><figure key={image} className={(i===0||i===3)?'wide':''}><img src={sitePath(image)} alt={`Integrated Urban District visualization ${String(i+1).padStart(2,'0')}`}/><figcaption>{['URBAN ORGANIZATION + STREET HIERARCHY','MIXED-USE STREET + PUBLIC FRONTAGE','RESIDENTIAL TYPOLOGY + ARCHITECTURAL SCALE','NEIGHBORHOOD PATTERN + LANDSCAPE'][i]}</figcaption></figure>)}</div></section>
  ...
  <a className="next-project" href={sitePath("/projects/terraced-community-park")}><span>Previous project</span><h2>Terraced Community Park ↗</h2></a>
</main>}
...
function AnimeVillage({p}:{p:Project}){return <main className="case thesis-case">
  ...
  <section className="thesis-image-section"><div className="section-label"><b>03</b><span>Site + Context</span></div><figure><img src={sitePath("/projects/anime-village/17.png")} alt="Anime Village site entrance and surrounding landscape"/><figcaption>Airport Road, Syria · Arrival edge and landscape context</figcaption></figure></section>
  <section className="thesis-image-section masterplan"><div className="section-label"><b>04</b><span>Masterplan</span></div><figure><img src={sitePath("/projects/anime-village/2.png")} alt="Anime Village aerial masterplan showing buildings, public spaces, landscape and circulation"/><figcaption>Overall organization · buildings · public realm · landscape · movement</figcaption></figure></section>
  <section className="thesis-pair"><div className="section-label"><b>05</b><span>Program</span></div><div className="pair-grid"><figure><img src={sitePath("/projects/anime-village/6.png")} alt="Academic and cultural building cluster"/><figcaption>Cultural and educational program</figcaption></figure><div className="program-list"><span>01 / Cultural districts</span><span>02 / Exhibition + learning</span><span>03 / Performance + event spaces</span><span>04 / Retail + hospitality</span><span>05 / Landscape + public realm</span><span>06 / Entertainment attractions</span></div></div></section>
  <section className="thesis-image-section landscape"><div className="section-label"><b>06</b><span>Landscape + Public Life</span></div><figure><img src={sitePath("/projects/anime-village/18.png")} alt="Landscaped pedestrian route at Anime Village"/><figcaption>Pedestrian route · planting · lighting · gathering</figcaption></figure></section>
  <section className="renders"><header className="gallery-head"><div className="section-label"><b>07</b><span>Selected Renders</span></div><p>Architecture / public space / atmosphere</p></header><div className="render-gallery">{p.gallery.slice(2).map((image,i)=><figure key={image} className={(i%7===0||i%7===4)?'wide':''}><img src={sitePath(image)} alt={`Anime Village architectural visualization ${String(i+1).padStart(2,'0')}`}/><figcaption>VIEW / {String(i+1).padStart(2,'0')}</figcaption></figure>)}</div></section>
  ...
  <a className="next-project" href={sitePath("/projects/fire-station")}><span>Next project</span><h2>Fire Station ↗</h2></a>
</main>}
